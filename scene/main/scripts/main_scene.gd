...
func _red_touched():
	...
	var bound_x = bounds.size.x * .25
	var bound_y = bounds.size.y * .25
	
	var new_x = randf_range(bounds.position.x, bounds.position.x + bounds.size.x)
	var new_y = randf_range(bounds.position.y, bounds.position.y + bounds.size.y)
